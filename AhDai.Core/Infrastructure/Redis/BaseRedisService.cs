...
public class BaseRedisService : IBaseRedisService, IDisposable
{
    ...
    readonly RedisOptions _options;
    ...
    public BaseRedisService(ILogger<BaseRedisService> logger, IOptions<RedisOptions> options)
    {
        _logger = logger;
    ...
    /// <returns></returns>
    public virtual string CreateConfiguration(RedisOptions? options = null)
    {
        var ops = options ?? _options;
        return CreateConfiguration(ops.Host, ops.Port, ops.Password, ops.AbortConnect);
    }
    ...
    /// <param name="options">自定义配置</param>
    /// <returns></returns>
    public virtual IConnectionMultiplexer CreateConnection(RedisOptions? options = null)
    {
        var ops = options ?? _options;
        var configString = CreateConfiguration(ops);
    ...
    /// <param name="options">自定义配置</param>
    /// <returns></returns>
    public virtual IConnectionMultiplexer GetConnection(RedisOptions? options = null)
    {
        var ops = options ?? _options;
        var configString = CreateConfiguration(ops);
        var key = $"{ops.Host}:{ops.Port}:{ops.Password}:{ops.AbortConnect}";
    ...
    /// <param name="options">自定义配置</param>
    /// <returns></returns>
    public virtual IDatabase GetDatabase(int db = -1, object? asyncState = null, RedisOptions? options = null)
    {
        var ops = options ?? _options;
        var connection = GetConnection(ops);
        return connection.GetDatabase(db > -1 ? db : ops.Database, asyncState);
    }
