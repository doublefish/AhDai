...
/// WeChatService
/// </summary>
internal class WeChatService<TConfig, TConfigProvider>(IBaseRedisService redisService, TConfigProvider configProvider, IHttpClientFactory httpClientFactory)
    : BaseService<TConfig, TConfigProvider>(configProvider, httpClientFactory)
    , IWeChatService<TConfig>
    where TConfig : WeChatConfig
    where TConfigProvider : IBaseConfigProvider<TConfig>
{
    ...
    public async Task<AccessTokenOutput> GetAccessTokenAsync(bool enableCache = false)
    {
        ...
        if (enableCache)
        {
            ...
            var key = $"AhDai:WeChat:{config.AppId}:AccessToken";
        ...
        return await SendAsync<AccessTokenOutput>(config, HttpMethod.Get, url);
    }
    ...
    public async Task<AccessTokenOutput> GetStableAccessTokenAsync(bool enableCache = false)
    {
        ...
        {
            var rdb = _redisService.GetDatabase();
            var key = $"AhDai:WeChat:{config.AppId}:StableAccessToken";
            var value = await rdb.StringGetAsync(key);
            if (value.HasValue)
            {
                var res = JsonUtil.Deserialize<AccessTokenOutput>(value.ToString());
        ...
        return await SendAsync<AccessTokenOutput, Dictionary<string, object>>(config, HttpMethod.Post, url, input);
    }
    ...
    public async Task<TicketOutput> GetTicketAsync(string? type = null, bool enableCache = false)
    {
        ...
        {
            var rdb = _redisService.GetDatabase();
            var key = $"AhDai:WeChat:{config.AppId}:Ticket:{type}";
            var value = await rdb.StringGetAsync(key);
            if (value.HasValue)
            {
        ...
        return await SendAsync<TicketOutput>(config, HttpMethod.Get, url);
    }
    ...
    public async Task<AuthTokenOutput> GetAuthTokenByCodeAsync(string code)
    {
        ...
        return await SendAsync<AuthTokenOutput>(config, HttpMethod.Get, url);
    }
    ...
    public async Task<UserOutput> GetUserByAccessTokenAsync(string token, string openId)
    {
        ...
        return await SendAsync<UserOutput>(config, HttpMethod.Get, url);
    }

    protected Task<TOutput> SendAsync<TOutput>(TConfig config, HttpMethod method, string url)
        where TOutput : BaseOutput
        => SendAsync<TOutput, string>(config, method, url, null);

    protected async Task<TOutput> SendAsync<TOutput, TInput>(TConfig config, HttpMethod method, string url, TInput? input)
        where TOutput : BaseOutput
        where TInput : class
    {
        ...
        var res = await response.Content.ReadFromJsonAsync<TOutput>() ?? throw new Exception("请求微信服务发生异常：解析响应结果失败，请联系管理员");
        if (res.ErrorCode.HasValue && res.ErrorCode != 0) throw new Exception($"请求微信服务发生异常：[{res.ErrorCode}]{res.ErrorMessage}，请联系管理员");
        return res;
    }
}
