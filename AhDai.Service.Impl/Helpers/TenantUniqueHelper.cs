...
internal class UniqueHelper<TEntity> where TEntity : class, IBaseEntity
{
	...
	public async Task<TResult> ExistAsync<TResult>(Func<Task<TResult>> success, Expression<Func<TEntity, string>> selector, string value, long id, string? errorMessage = null)
	{
		...
		var RedisKey = MyConst.Redis.GenerateKey<TEntity>("Unique", member.Member.Name, RedisSuffix ?? "");
		var RedisKey2 = $"{RedisKey}:Temp";

		var step = 0;
		try
		{
			var exist = await RedisDb.HashGetAsync(RedisKey, value);
			if (exist.HasValue && !exist.Equals(id))
			{
				throw new ArgumentException(errorMessage);
			}

			var increment = await RedisDb.HashIncrementAsync(RedisKey2, value, 1);
			step++;
			if (increment > 1)
			{
				throw new ArgumentException(errorMessage);
			...
			if (entity != null)
			{
				await RedisDb.HashSetAsync(RedisKey, value, entity.Id);
				throw new ArgumentException(errorMessage);
			}
		...
		finally
		{
			if (step > 0)
			{
				var temp = await RedisDb.HashDecrementAsync(RedisKey2, value, 1);
				// 极小概率的并发风险
				if (temp == 0) await RedisDb.HashDeleteAsync(RedisKey2, value);
			}
		}
