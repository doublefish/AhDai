...
using Adai.Security.Extensions;
using System;
using System.IO;
...
namespace Adai.Security.Utilities
{
	/// <summary>
	/// RSAHelper
	/// </summary>
	public static class RSAHelper
	{
		...
		public static string EncryptByXml(string original, string key, StringType stringType = StringType.Base64, Encoding encode = null)
		{
			...
			{
				StringType.Hex => Adai.Utilities.HexHelper.ToHexString(bytes),
		...
		public static string DecryptByXml(string ciphertext, string key, StringType stringType = StringType.Base64, Encoding encode = null)
		{
			...
			{
				case StringType.Hex:
					bytes = Adai.Utilities.HexHelper.ToBytes(ciphertext);
		...
		public static bool VerifyByXml(string sign, string data, string publicKey, string hashName = HashHalg.MD5, StringType stringType = StringType.Base64, Encoding encode = null)
		{
			...
			{
				StringType.Hex => Adai.Utilities.HexHelper.ToBytes(sign),
				_ => Adai.Utilities.Base64Helper.ToBytes(sign),
			};
			using var rsa = new RSACryptoServiceProvider();
		...
		{
			var buffer = Adai.Utilities.Base64Helper.ToBytes(publicKey);
		...
		{
			var buffer = Adai.Utilities.Base64Helper.ToBytes(privateKey);
			if (buffer == null)
			{
